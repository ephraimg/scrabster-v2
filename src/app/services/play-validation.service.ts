...
export class PlayValidationService {
    ...
    constructor(private boardService: BoardService) {
        this.getSquare = this.boardService.getSquare.bind(this.boardService);
    }
    ...
    isContiguous(play: Play) {
        ...
        if (this.isRowAligned(play)) {
            ...
            for (let col = sqs[0].col; col < sqs[sqs.length - 1].col; col++) {
                if (!this.getSquare(row, col).tile) { return false; }
            }
    ...
    getAllInRow(square: Square) {
        ...
        let row = square.row;
        let col = square.col;
        while (this.getSquare(row, ++col) && this.getSquare(row, col).tile) {
            squares.push(this.getSquare(row, col));
        }
        col = square.col;
        while (this.getSquare(row, --col) && this.getSquare(row, col).tile) {
            squares.unshift(this.getSquare(row, col));
        }
        return squares;
    };
    ...
    isConnected(play: Play, inputOrient?: string) {
        ...
        if (orient === 'vertical') {
            ...
            if (sqs[0].row > 0 && this.getSquare(sqs[0].row - 1, sqs[0].col).tile) {
                return true;
            }
            if (lastSq.row < 14 && this.getSquare(lastSq.row + 1, lastSq.col).tile) {
                return true;
            }
            for (let row = sqs[0].row; row < lastSq.row; row++) {
                const sq = this.getSquare(row, lastSq.col);
            ...
            let sqs = this.getPlaySquares(play);
            let lastSq = sqs[sqs.length - 1];
            if (sqs[0].col > 0 && this.getSquare(sqs[0].row, sqs[0].col - 1).tile) {
                return true;
            }
            if (lastSq.col < 14 && this.getSquare(lastSq.row, lastSq.col + 1).tile) {
                return true;
            }
            for (let col = sqs[0].col; col < lastSq.col; col++) {
                const sq = this.getSquare(lastSq.row, col);
                if (sq.tile && !play.placements.includes(sq)) {
                    return true;
                }
            }
            ...
            if (sq.row > 0 && this.getSquare(sq.row - 1, sq.col).tile) { return true; }
            if (sq.row < 14 && this.getSquare(sq.row + 1, sq.col).tile) { return true; }
            if (sq.col > 0 && this.getSquare(sq.row, sq.col - 1).tile) { return true; }
            if (sq.col < 14 && this.getSquare(sq.row, sq.col + 1).tile) { return true; }
        }
        return false;
    }
